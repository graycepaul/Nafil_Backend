...
--
-- One estate, four accounts (one per role), and sample passes/issues/
-- announcements so no screen opens empty. Trimmed down from the original
-- two-estate/nine-account seed to match the four accounts used for App
...
-- block below removes only the seeded rows (@nafil.test users and the
-- fixed estate UUID), nothing else.
--
...
-- Estate first, deliberately: visitor_logs.security_id has no ON DELETE
...
-- against it. Deleting the estate first cascades away visitor_passes/
...
delete from estates where id = '11111111-1111-1111-1111-111111111111';
